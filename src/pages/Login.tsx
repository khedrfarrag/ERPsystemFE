...
import { Lock, Mail, Sparkles, ShieldCheck, Sun, Moon, Eye, EyeOff, Store } from 'lucide-react';
import { useTheme } from '../context/ThemeContext';
import { RegisterStoreModal } from '../features/auth/components/RegisterStoreModal';
...
export const Login: React.FC = () => {
  ...
  return (
    <div className="min-h-screen bg-slate-100 dark:bg-slate-950 flex items-center justify-center p-4 select-none relative transition-colors duration-200" dir="rtl">
      ...
      <div className="max-w-md w-full bg-white dark:bg-slate-900 rounded-3xl shadow-2xl p-6 sm:p-8 border border-slate-200/80 dark:border-slate-800 text-slate-900 dark:text-white transition-colors">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          ...
          <div className="pt-2 text-center">
            <button
              ...
              className="inline-flex items-center gap-1.5 text-xs font-bold text-primary-600 dark:text-primary-400 hover:text-primary-700 dark:hover:text-primary-300 transition-colors py-1 cursor-pointer"
            >
              <Store className="w-3.5 h-3.5" />
              <span>ليس لديك متجر؟ سجل متجرك الجديد الآن مجاناً</span>
            </button>
          </div>
        </form>

        <div className="mt-6 pt-5 border-t border-slate-200 dark:border-slate-800">
          <p className="text-xs text-slate-600 dark:text-slate-300 font-bold mb-2.5 flex items-center gap-1.5">
          ...
          <div className="grid grid-cols-2 gap-2">
            <button
              ...
              className="px-2.5 py-2 bg-slate-100 dark:bg-slate-800 hover:bg-primary-50 dark:hover:bg-slate-700 text-slate-700 dark:text-slate-200 hover:text-primary-700 dark:hover:text-primary-400 rounded-xl text-xs font-semibold transition border border-slate-200 dark:border-slate-700 text-center"
            >
              المالك (Owner)
            </button>
            <button
            ...
            <button
              type="button"
              onClick={() => setDemo('merchant@retailos.com', 'RetailOS@Merchant2026!')}
              className="px-2.5 py-2 bg-amber-50 dark:bg-amber-950/40 hover:bg-amber-100 dark:hover:bg-amber-900/50 text-amber-800 dark:text-amber-300 rounded-xl text-xs font-semibold transition border border-amber-200 dark:border-amber-800/60 text-center"
            >
              تاجر الجملة (B2B)
            </button>
          </div>
