...
import { disbursePaymentFormSchema, type DisbursePaymentFormData } from '../types/suppliers.schemas';
import { useDisbursePaymentMutation } from '../api/useSuppliersMutations';
import type { Supplier } from '../types/suppliers.types';

interface DisbursePaymentModalProps {
  isOpen: boolean;
  onClose: () => void;
  supplier: Supplier | null;
}

export const DisbursePaymentModal: React.FC<DisbursePaymentModalProps> = ({
  isOpen,
  onClose,
  supplier,
}) => {
  const disburseMutation = useDisbursePaymentMutation();
  ...
  const {
    ...
    reset,
    formState: { errors },
  } = useForm<DisbursePaymentFormData>({
    resolver: zodResolver(disbursePaymentFormSchema),
    defaultValues: {
      amount: 0,
      paymentMethod: 'Cash',
      referenceNumber: '',
  ...
  const onSubmit = async (data: DisbursePaymentFormData) => {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm animate-fade-in">
      <div className="bg-white dark:bg-slate-800 rounded-3xl border border-slate-200 dark:border-slate-700 shadow-2xl w-full max-w-md overflow-hidden flex flex-col">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="p-6 space-y-4">
          {/* Current Payables Highlight */}
          <div className="p-3.5 rounded-2xl bg-slate-900 text-white flex items-center justify-between shadow-inner">
            <div>
              <span className="text-[11px] text-slate-600 dark:text-slate-300 font-bold block">إجمالي المستحقات للمورد</span>
            ...
            <button
              type="button"
              onClick={() => setValue('amount', supplier.currentBalance)}
              className="px-2.5 py-1 text-xs font-bold bg-blue-600 hover:bg-blue-500 text-white rounded-lg transition-colors"
            >
              سداد المبلغ كاملاً
            </button>
          </div>
          ...
          <div>
            ...
            >
              <option value="Cash">نقدي (صرف من الخزينة/الدرج)</option>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 dark:text-slate-300 mb-1">
              رقم الإيصال / مرجع الصرف (اختياري)
            </label>
            <input
              type="text"
              {...register('referenceNumber')}
              placeholder="مثال: PAY-00123"
          ...
          <div>
            ...
            <input
              type="text"
              {...register('notes')}
              placeholder="مثال: سداد دفعة مقابل فاتورة رقم 102"
          ...
          <div className="pt-2 flex items-center justify-end gap-2 border-t border-slate-100 dark:border-slate-700">
            ...
            <button
              ...
              className="flex items-center gap-1.5 px-6 py-2.5 text-xs font-extrabold text-white bg-blue-600 hover:bg-blue-700 rounded-xl shadow-md disabled:opacity-40"
            >
              {disburseMutation.isPending ? <Loader2 className="w-4 h-4 animate-spin" /> : <CheckCircle2 className="w-4 h-4" />}
              <span>تأكيد صرف الدفعة</span>
            </button>
          </div>
        </form>
      </div>
